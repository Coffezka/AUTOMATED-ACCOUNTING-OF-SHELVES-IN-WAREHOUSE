...
flags.DEFINE_string('video', './data/video/pasta.mp4', 'path to input video or set to 0 for webcam')
flags.DEFINE_string('output', './data/video/output.MOV', 'path to output video')
flags.DEFINE_string('output_format', 'X264', 'codec used in VideoWriter when saving video to file')
...
def main(_argv):
    ...
    while True:
        ...
        if FLAGS.count:
            ...
            products = [*range(1, len(counted_classes)+1, 1)]

            amount = []
            for key in counted_classes.keys():
                amount.append(counted_classes[key])

            products_dict = {'products': products, "amount": amount}

            response = requests.post('http://127.0.0.1:8000/state/', json = products_dict)
